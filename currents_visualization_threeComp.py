...
def plotCurrentscape(voltage, currents):	
	...
	plot(t, voltage, color='black',lw=0.5)
...
# 	plot(0.005*ones(len(nnPD)),color='grey', ls=':',lw=1)
# 	plot(0.5*ones(len(nnPD)),color='grey', ls=':',lw=1)
# 	plot(50.*ones(len(nnPD)),color='grey', ls=':',lw=1)
# 	yscale('log')
# 	ylim(0.00001,1500)
# 	xlim(0,xmax)
# 	axis('off') 
...
# 	colormap = imshow(im0[::1,::1],interpolation='nearest',aspect='auto',cmap=elcolormap) 
# 	ylim(2*resy,0)
